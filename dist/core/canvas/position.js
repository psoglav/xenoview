...
export class Position extends VElement {
    get color() {
        return new Color(this.chart.options.candles.colors[this.options.side === 'buy' ? 'higher' : 'lower']).alpha(this.options.disabled ? 0.5 : 1);
    }
    get fillColor() {
    ...
    constructor(canvas, options) {
        ...
        this.setPosition(this.chart.canvasRect.width - 200, this.chart.normalizeToY(this.options.deltaPrice + this.options.price));
        const padding = {
            x: 8,
            y: 4
        };
    ...
    update() {
        if (this.options.status !== 'working')
            return;
        this.setPosition(this.chart.canvasRect.width - 200, this.chart.normalizeToY(this.options.deltaPrice + this.options.price));
        let xshift = 0;
        let height = 0;
